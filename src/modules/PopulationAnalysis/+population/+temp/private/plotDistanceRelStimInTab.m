function plotDistanceRelStimInTab(parent, mergedTable, kvargs)
	%%PLOTDISTANCERERLSTIMINTAB Plot movement distance in a relative-to-onset window across bout bins
	%
	%   plotDistanceRelStimInTab(parent, mergedTable)
	%
	...
	%
	%   For each stimulus bout, this computes movement distance in a window
	%   relative to stimulus onset (t=0) from centerpoint X/Y:
	%       distance = sum of frame-to-frame displacement in the relative window
	%
	%   Units are cm (assuming X center and Y center are in cm).
	%
	%   Inputs:
	...
	for stimsetIdx = 1:nstimsets
		...
		for g = 1:length(groups)
			...
			for stimIdx = 1:length(thisStimSet)
				...
				% Per-bout distance (cm) in relative-to-onset window.
				perBoutDistance = NaN(bi.nBouts, size(xMat, 2));
				for boutIdx = 1:bi.nBouts
					...
					perBoutDistance(boutIdx, :) = totalDist;
				end
				...
				for binIdx = 1:bi.nBins
					...
					bm = mean(perBoutDistance(bs:be, :), 1, 'omitnan');
					vv = bm(~isnan(bm));
		...
		ylabel(a, sprintf('Movement Distance (%s s rel. onset)\n(cm)', windowLabel));
		grid(a, 'on');
	end
