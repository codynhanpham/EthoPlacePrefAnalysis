function f = distFromMidlineByBout(standardizedTable, kvargs)
    %%DISTFROMMIDLINEBYBOUT Plot cumulative distance from midline aligned by bout onset, binned over time
    %
        ...
        for strainIdx = 1:nstrains
            ...
            for genotypeIdx = 1:ngenotypes
                ...
                for sexPlotIdx = 1:nsexes
                    ...
                    % Add reference line at y=0 and labels
                    yline(a, 0, 'k--', 'LineWidth', 1);
                    
                    title(a, sprintf('[%s]  %s  %s (Bin = %.2fs, Bout Range = %s)', strjoin(thisStimSet, ' / '), strain, genotype, kvargs.BinWidth, percentRangeStr), 'Interpreter', 'none');
