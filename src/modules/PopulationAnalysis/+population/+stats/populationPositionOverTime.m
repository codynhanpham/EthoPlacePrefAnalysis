function standardizedTables = populationPositionOverTime(ethovisionTrials, stimuliDir, masterMetadataTable, kvargs)
    ...
    %               - fps: The standardized framerate of centerpointData and bodyparts.
    %               - px2cm: The pixels-to-centimeters conversion factor used for the standardized coordinates.
    %               - bodyparts: Optional table containing standardized Trial time, Stimulus name, and X/Y body-part columns; no center statistics.
    ...
    for i = 1:ntrials
        ...
        if ~isempty(bodyparts) && isfield(bodyparts, 'data') && ...
            ...
            bodypartTable = bodyparts.data;
            bodypartPx2cm = bodyparts.px2cm;
            if ~isscalar(bodypartPx2cm) || ~isfinite(bodypartPx2cm) || ...
                    ~isscalar(centerpointData.px2cm) || ~isfinite(centerpointData.px2cm) || ...
                    ~isequaln(bodypartPx2cm, centerpointData.px2cm)
                error('population:stats:populationPositionOverTime:BodypartPx2cmMismatch', ...
                    'Bodypart and centerpoint pixels-to-centimeters conversion factors must match for trial %d.', i);
            end
            bodypartData = table(standardizedTime, standardizedStims, ...
                'VariableNames', {'Trial time', 'Stimulus name'});
            bodypartXVariables = string(bodypartTable.Properties.VariableNames( ...
                startsWith(bodypartTable.Properties.VariableNames, 'X ')));
            for bodypartIdx = 1:numel(bodypartXVariables)
                ...
                if ~ismember(yVariable, bodypartTable.Properties.VariableNames)
                    continue;
                end
                bodypartX = interp1(bodypartTable{:, 'Trial time'}, ...
                    bodypartTable{:, xVariable}, standardizedTime, kvargs.Interpolation, 'extrap');
                bodypartY = interp1(bodypartTable{:, 'Trial time'}, ...
                    bodypartTable{:, yVariable}, standardizedTime, kvargs.Interpolation, 'extrap');
        ...
        if width(bodypartData) > 2
            ...
            if isempty(stimtables(stimfileName).bodyparts)
                stimtables(stimfileName).bodyparts = bodypartData;
            else
                stimtables(stimfileName).bodyparts = outerjoin( ...
                    stimtables(stimfileName).bodyparts, bodypartData, ...
                    'Keys', {'Trial time', 'Stimulus name'}, ...
                    'MergeKeys', true);
            end
            if isnan(stimtables(stimfileName).px2cm)
                stimtables(stimfileName).px2cm = bodypartPx2cm;
            elseif ~isequaln(stimtables(stimfileName).px2cm, bodypartPx2cm)
                error('population:stats:populationPositionOverTime:InconsistentPx2cm', ...
                    'Inconsistent pixels-to-centimeters conversion factors detected for stimulus file %s.', stimfileName);
            end
        end

        if isnan(stimtables(stimfileName).px2cm)
            stimtables(stimfileName).px2cm = centerpointData.px2cm;
        elseif ~isequaln(stimtables(stimfileName).px2cm, centerpointData.px2cm)
            error('population:stats:populationPositionOverTime:InconsistentPx2cm', ...
                'Inconsistent pixels-to-centimeters conversion factors detected for stimulus file %s.', stimfileName);
        end
    ...
    for i = 1:length(stimfileNames)
        ...
        bodypartTable = stimtables(stimfileName).bodyparts;
        if width(bodypartTable) > 2
            stimtables(stimfileName).bodyparts = mergeBodypartVariables(bodypartTable);
        end
