function f = headVelocityByBout(standardizedTable, kvargs)
    %%HEADVELOCITYBYBOUT Plot instantaneous head speed aligned by bout onset, binned over time
    %
    %   This function computes the instantaneous movement speed of the head (defined as the
    %   midpoint of the Ear_left and Ear_right bodyparts), aligned to bout onset, for each
    %   bout of stimulus presentation in the provided standardizedTable.
    %   Speed is computed from the raw XY trajectory of the ear midpoint and kept in
    %   PHYSICAL units (cm/s, via px2cm) — no per-replicate normalization. This makes the
    %   plot a direct visual counterpart of graphics.headReactionTimeByBout(), which finds
    %   the latency of the peak speed within the post-onset window.
    %
    %   Each replicate's speed is baseline-corrected by subtracting its mean speed within
    %   BaselineWindow (default [-1, 0] s relative to bout onset), so the trace shows the
    %   stimulus-evoked speed change. An optional moving-average smoothing (SmoothWindow,
    %   in seconds) can be applied to tame per-frame noise.
    %
    %   f = graphics.headVelocityByBout(standardizedTable, kvargs)
    %
    %   Inputs:
    ...
    %       'BinWidth' : Scalar double specifying the width of time bins (in seconds) for averaging speed data. Default is NaN, which uses the smallest time resolution available. Set to 0 or NaN for no binning, or Inf for one single bin over the whole ResponseWindow.
    ...
    %       'BaselineWindow' : 1x2 double array specifying the time window (in seconds, relative to bout onset) whose mean speed is subtracted from each replicate's speed trace. Default is [-1, 0]. Must lie within ResponseWindow and end at or before 0.
    %       'SmoothWindow' : Scalar double specifying the moving-average smoothing window in seconds. Default is NaN (no smoothing). 0 or NaN disables smoothing.
    %
    ...
    %
    %   See also: population.stats.populationPositionOverTime, graphics.headReactionTimeByBout, graphics.headAccelerationByBout, graphics.headMovementByBout
    ...
    arguments
        ...
        kvargs.BaselineWindow (1,2) double = [-1, 0] % in seconds, relative to bout onset. Mean speed within this window is subtracted per replicate. Must lie within ResponseWindow and end at or before 0.
        kvargs.SmoothWindow (1,1) double = NaN % in seconds, moving-average smoothing window; 0 or NaN disables smoothing
    ...
    % At the onset of each bout, track the instantaneous head speed (from the ear-midpoint
    % XY trajectory) over time, baseline-corrected to the pre-onset BaselineWindow.
    % Speed is kept in physical units (cm/s via px2cm, or px/s if px2cm is unavailable) —
    % no normalization, so magnitudes are directly interpretable and comparable with
    % graphics.headReactionTimeByBout()'s peak-speed search.
    ...
    f = figure('Name', sprintf("Head Velocity By Bout (Bin Size: %.3f sec)", kvargs.BinWidth), 'Position', figPos, 'NumberTitle', 'off');
    ...
    for stimsetIdx = 1:nstimsets
        ...
        if isfinite(thisPx2cm)
            speedUnitScale = thisPx2cm; % px/s -> cm/s
            unitLabel = 'cm/s';
        else
            speedUnitScale = 1; % stay in px/s
            unitLabel = 'px/s (px2cm unavailable)';
        end
            ...
            genotypeSexData = dictionary(); % Use dictionary to handle stimulus names with spaces
                ...
                for sexIdx = 1:nsexes
                    ...
                    for stimIdx = 1:length(stimsBouts.keys()) % Stim bout key order matches thisStimSet order
                        ...
                        allBinnedSpeeds = []; % will be nBins x nReplicates x nBouts
                        ...
                        for boutIdx = 1:nBouts
                            ...
                            % Baseline-correct: subtract each replicate's mean speed within BaselineWindow,
                            % so the trace shows stimulus-evoked speed change relative to pre-onset level
                            ...
                            % Bin the speedSeries into fixed bins anchored at time 0
                            binEdgesAbsolute = binEdgesRelative + boutStartTime;
                            [~, ~, binIndices] = histcounts(timeVector, binEdgesAbsolute);

                            binnedBoutSpeeds = NaN(nBins, size(speedSeries, 2)); % nBins x nReplicates
                            for binIdx = 1:nBins
                                binMask = binIndices == binIdx;
                                if any(binMask)
                                    binnedBoutSpeeds(binIdx, :) = mean(speedSeries(binMask, :), 1, 'omitnan');
                                end
                            end

                            allBinnedSpeeds = cat(3, allBinnedSpeeds, binnedBoutSpeeds); % nBins x nReplicates x nBouts
                        end

                        nreplicates = size(allBinnedSpeeds, 2);
                        ...
                        meanSpeed = squeeze(mean(allBinnedSpeeds, 3, 'omitnan')); % nBins x nReplicates
                        meanAcrossReplicates = mean(meanSpeed, 2, 'omitnan'); % nBins x 1
                        semAcrossReplicates = std(meanSpeed, 0, 2, 'omitnan') / sqrt(size(meanSpeed, 2));
                ...
                for sexPlotIdx = 1:nsexes
                    ...
                    for stimIdx = 1:length(stimsBouts.keys())
                        ...
                        if isKey(genotypeSexData, compositeKey)
                            ...
                            upperBound = meanSpeed + semSpeed;
                            lowerBound = meanSpeed - semSpeed;
                            ...
                            lineHandle = plot(a, binTimeCenters, meanSpeed, ...
                                'LineStyle', lineStyle, ...
                                'Color', lineColor, ...
                                'LineWidth', 2, ...
                ...
                ylabel(a, sprintf('Head Speed (baseline-corrected, %s)\nBaseline Window = [%.2f, %.2f] s', unitLabel, kvargs.BaselineWindow(1), kvargs.BaselineWindow(2)));
...
function smoothed = smoothBySeconds(values, timeAxis, windowSec)
    % Causal-free centered moving-average smoothing along rows (time dimension).
